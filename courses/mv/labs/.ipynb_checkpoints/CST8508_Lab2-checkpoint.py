...
def calculate_and_display_histogram(image_path):
    ...
    # Requirement: Convert to grayscale for histogram calculation
    # Grayscale simplifies histogram to single channel (0-255 intensity values)
    # 要求：转换为灰度图以计算直方图
    # 灰度图简化直方图为单通道（0-255 强度值）
    gray = cv2.cvtColor(img, cv2.COLOR_BGR2GRAY)
    
    # Requirement: Calculate histogram
    # Histogram shows distribution of pixel intensities in the image
    # 要求：计算直方图
    # 直方图显示图像中像素强度的分布
    ...
    # ranges=[0,256]: 像素值范围
    img_histogram = cv2.calcHist([gray], [0], None, [256], [0, 256])
    
    # Requirement: Display the histogram
    # Visualize pixel intensity distribution to understand image characteristics
    # 要求：显示直方图
    # 可视化像素强度分布以了解图像特征
    ...
    # 显示直方图
    plt.subplot(1, 2, 2)
    plt.plot(img_histogram, color='black')
    plt.title('Histogram')
