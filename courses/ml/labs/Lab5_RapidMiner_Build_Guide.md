...
**Main process（主流程，分两阶段运行）：**

> ⚠️ **两阶段对比法**：先不加 Sample 算子运行一次（Phase A），再加上 Sample 算子运行一次（Phase B），对比重采样前后的分类效果。

**Phase A — Before Resampling（不做重采样，直连）：**

```mermaid
flowchart LR
    A["Retrieve Diabetes"] -- "out" --> B["DataPrep_Diabetes"]
    B -- "out" --> D["PreparedSample"]
...
**Phase B — After Resampling（加入 Sample 算子）：**
...
flowchart LR
    ...
    SR --> N2B["Nominal to Binominal<br>class"]
    N2B --> NORM["Normalize<br>Z-transformation"]
    NORM --> out["out"]
```
...
| 参数 Parameter            | 值 Value         | 含义 Meaning                                                                     |
| ------------------------- | ---------------- | -------------------------------------------------------------------------------- |
| `attribute filter type` | subset           | 仅对 `class` 列做转换                                                           |
| `attributes`            | `class`          | 选择 label 列                                                                   |

> 将 `class` 列从 polynominal 转为 binominal 类型。SVM、Logistic Regression 等二分类算子要求 label 必须是 binominal，否则会报错。
...
#### STEP 3.5 — Phase A: Before Resampling（先运行一次不做重采样）

> 📌 **目的**：获取不做重采样时的分类基准结果，用于与 Phase B 对比。

**操作步骤：**

1. 完成 STEP 3 (DataPrep_Diabetes) 后，**暂时不添加 Sample 算子**
2. 将 DataPrep_Diabetes 的 `out` 直接连接到 PreparedSample 的 `inp`
3. 按照 STEP 5、STEP 6 建好 PreparedSample 和 Classifications 子流程
4. 运行 (Ctrl+R)
5. 截图所有 5 个分类器的 Performance 结果，保存为 `task2_05_knn_before.png` 等
6. 这些结果就是 **Before Resampling** 的基准

---

#### STEP 4 — Phase B: Sample (Bootstrapping)（加入重采样算子）

> 📌 **目的**：加入 Sample (Bootstrapping) 算子平衡类别，再次运行并对比。

**操作步骤：**

1. 断开 DataPrep_Diabetes `out` → PreparedSample `inp` 的直连线
2. 在两者之间插入 Sample (Bootstrapping) 算子
3. 连接：DataPrep_Diabetes `out` → Sample `inp`，Sample `exa` → PreparedSample `inp`
4. 运行 (Ctrl+R)
5. 截图所有 5 个分类器的 Performance 结果，保存为 `task2_05_knn_after.png` 等
...
> 目标结果：两类各 384，共 768 个实例。负类从 500 下采样到 384，正类从 268 通过有放回采样上采样到 384。
...
| ✅ | `task2_03_before.png`                | Class distribution before resampling (500 neg / 268 pos)                                                     |
| ✅ | `task2_03_after.png`                 | Class distribution after resampling (384 neg / 384 pos)                                                      |
| ✅ | `task2_04_split.png`                 | Inside PreparedSample: Split Data parameters (0.7/0.3, seed=730)                                             |
|    | **Phase A — Before Resampling** |                                                                                                              |
| ✅ | `task2_05_knn_before.png`            | kNN Performance results (before resampling)                                                                  |
| ✅ | `task2_05_nb_before.png`             | Naive Bayes Performance results (before resampling)                                                          |
| ✅ | `task2_05_svm_before.png`            | SVM Performance results (before resampling)                                                                  |
| ✅ | `task2_05_lr_before.png`             | Logistic Regression Performance results (before resampling)                                                  |
| ✅ | `task2_06_stacking_before.png`       | Stacking Performance results (before resampling)                                                             |
|    | **Phase B — After Resampling**  |                                                                                                              |
| ✅ | `task2_05_knn_after.png`             | kNN Performance results (after resampling)                                                                   |
| ✅ | `task2_05_nb_after.png`              | Naive Bayes Performance results (after resampling)                                                           |
| ✅ | `task2_05_svm_after.png`             | SVM Performance results (after resampling)                                                                   |
| ✅ | `task2_05_lr_after.png`              | Logistic Regression Performance results (after resampling)                                                   |
| ✅ | `task2_06_stacking_setup.png`        | Inside Classifications: Stacking operator with 3 base learners + LR meta                                     |
| ✅ | `task2_06_stacking_after.png`        | Stacking Performance results (after resampling)                                                              |
