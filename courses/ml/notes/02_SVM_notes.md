...
The projection of any vector or another vector is called dot-product.
...
```
假设训练好的 SVM 得到：
  权重向量 w = [2, 1]     （垂直于决策边界的方向）
  偏置 b = -5

决策函数: f(x) = w·x + b = w₁x₁ + w₂x₂ + b

── 为什么 w·x + b = 0 是一条线？──
  "点积"就是对应元素相乘再相加，展开就是普通乘法:
    w·x + b = 0
    w₁x₁ + w₂x₂ + b = 0        ← 这就是直线方程
    2x₁ + 1x₂ + (-5) = 0
    x₂ = -2x₁ + 5               ← 熟悉的 y = kx + c 形式
    斜率 k = -w₁/w₂ = -2，截距 c = -b/w₂ = 5

  同理:
    3D → w₁x₁ + w₂x₂ + w₃x₃ + b = 0 → 一个平面
    nD → w₁x₁ + w₂x₂ + ... + wₙxₙ + b = 0 → 超平面
...
**💡 提示:** 点积 w·x 的值就是 x 在 w 方向上的投影长度，值越大离边界越远
...
对于所有蓝色点（正类 yᵢ = +1）:
  w·xᵢ + b ≥ +1     ← 正类点必须在 L₁ 外面或上面

对于所有红色点（负类 yᵢ = -1）:
  w·xᵢ + b ≤ -1     ← 负类点必须在 L₂ 外面或下面
...
假设 w = [2, 1], b = -5
  ...
  蓝色点 x₁ = [4, 2], y₁ = +1:
    y₁(w·x₁ + b) = (+1)(2×4 + 1×2 - 5) = (+1)(5) = 5 ≥ 1 ✓

  红色点 x₂ = [1, 1], y₂ = -1:
    y₂(w·x₂ + b) = (-1)(2×1 + 1×1 - 5) = (-1)(-2) = 2 ≥ 1 ✓
```

**💡 提示:** yᵢ 的巧妙之处在于把两种约束统一成一个公式，简化了优化问题
...
两条平行超平面 (w·x + b = c₁) 和 (w·x + b = c₂) 之间的距离公式:
  d = |c₁ - c₂| / ||w||
...
如果 w = [1, 0.5]（更小的 w）:
  ...
  间隔 d = 2 / 1.118 ≈ 1.789   ← 间隔更大了！

── 关键结论 ──
  ||w|| 越小  →  间隔 d = 2/||w|| 越大
  所以最大化间隔 ⟺ 最小化 ||w||
  实际优化时，最小化 ½||w||²（去掉根号，方便求导）
```

**💡 提示:** 最大化间隔等价于最小化 ½||w||²，这就是 SVM 的优化目标
...
═══════════════════════════════════════════════════
  最小化:  ½||w||² = ½(w₁² + w₂²)
  约束:    yᵢ(w·xᵢ + b) ≥ 1    对每个样本 i

这是一个"带约束的优化问题"，用拉格朗日乘子法求解。

═══════════════════════════════════════════════════
第2步：构建拉格朗日函数
═══════════════════════════════════════════════════
为每个约束引入一个拉格朗日乘子 αᵢ ≥ 0:

  L(w, b, α) = ½||w||² - Σ αᵢ [yᵢ(w·xᵢ + b) - 1]

展开:
  L = ½||w||² - Σ αᵢyᵢ(w·xᵢ) - Σ αᵢyᵢb + Σ αᵢ

═══════════════════════════════════════════════════
第3步：对 w 和 b 求偏导，令其等于0
═══════════════════════════════════════════════════
  ∂L/∂w = 0  →  w = Σ αᵢyᵢxᵢ    ← w 由支持向量决定！
  ∂L/∂b = 0  →  Σ αᵢyᵢ = 0

关键发现: w 是训练样本的加权组合，权重就是 αᵢyᵢ

═══════════════════════════════════════════════════
第4步：转化为对偶问题（只含 α）
═══════════════════════════════════════════════════
把 w = Σαᵢyᵢxᵢ 代回 L，消去 w 和 b:

  最大化:  W(α) = Σ αᵢ - ½ Σᵢ Σⱼ αᵢαⱼyᵢyⱼ(xᵢ·xⱼ)
  约束:    αᵢ ≥ 0,  Σ αᵢyᵢ = 0

注意: 这里只需要计算 xᵢ·xⱼ（点积），这就是核技巧的入口！
      把 xᵢ·xⱼ 替换为 K(xᵢ, xⱼ) 就变成了非线性 SVM
...
用二次规划 (QP) 求解器解出:
  α₁ = 0.25,  α₂ = 0,  α₃ = 0.25,  α₄ = 0
  (α₂=0, α₄=0 说明它们不是支持向量)

第6步: 算出 w:
  w = Σ αᵢyᵢxᵢ
    = 0.25×(+1)×[1,1] + 0×(+1)×[2,2] + 0.25×(-1)×[-1,-1] + 0×(-1)×[-2,-2]
...
第8步: 最终决策函数:
  f(x) = w·x + b = 0.5x₁ + 0.5x₂ + 0
       = 0.5x₁ + 0.5x₂

验证:
  f(x₁) = 0.5×1 + 0.5×1 = 1 ≥ 0  → 正类 ✓
  f(x₃) = 0.5×(-1) + 0.5×(-1) = -1 < 0  → 负类 ✓

间隔: d = 2/||w|| = 2/√(0.25+0.25) = 2/√0.5 ≈ 2.83
```

**💡 提示:** 实际中 sklearn 用 SMO 算法（比 QP 更高效）来求解 αᵢ，但原理一样
...
硬间隔（MMC）:
  最小化 ½||w||²
  约束:  yᵢ(w·xᵢ + b) ≥ 1    对所有样本

问题：如果数据有噪声或重叠，上面的约束无解！

解决：引入松弛变量 ξᵢ（xi），允许部分点越过边界:
...
── 参数 C 的含义 ──
  C 控制"间隔宽度" vs "误分类惩罚"的权衡:

  ┌──────────────┬────────────────────────────┐
  │   C 值       │   效果                     │
  ├──────────────┼────────────────────────────┤
  │ C 很大(如1000)│ 严格分类，窄间隔，过拟合   │
  │ C 适中(如1.0) │ 平衡（sklearn默认值）      │
  │ C 很小(如0.01)│ 宽松分类，宽间隔，欠拟合   │
  └──────────────┴────────────────────────────┘

  C 大 → ξᵢ 的惩罚大 → 不允许误分类 → 接近硬间隔
  C 小 → ξᵢ 的惩罚小 → 容忍误分类   → 更宽的间隔

── 数值示例 ──
假设某个红色点 x₃ 跑到了蓝色那边:
  y₃(w·x₃ + b) = -0.5  （违反了 ≥ 1 的约束）
  松弛量 ξ₃ = 1 - (-0.5) = 1.5

  当 C = 10 时:  惩罚 = 10 × 1.5 = 15   ← 惩罚大
  当 C = 0.1 时: 惩罚 = 0.1 × 1.5 = 0.15 ← 惩罚小
```
...
- 核技巧 (kernel trick)：用 K(x, y) 直接算出高维点积，省去映射步骤
...
═══════════════════════════════════════════════
公式: K(x, y) = (γ × x·y + r)^d
  γ (gamma) = 默认 1/n_features = 1/2 = 0.5
  r (coef0) = 默认 0
  d (degree) = 默认 3

计算:
  x·y = 1×3 + 2×1 = 5
  K(x, y) = (0.5 × 5 + 0)³ = (2.5)³ = 15.625

等价于把 [x₁, x₂] 映射到包含 x₁³, x₁²x₂, x₁x₂², x₂³ 等项的高维空间
  → degree=2: [1,2] 映射为 [1, 2, 4, 2√2, √2, 2√2] (6维)
  → degree=3: 映射到更高维（10维+）
  → 核技巧: 不需要真的算出这些，K(x,y) 直接给出结果

═══════════════════════════════════════════════
3. RBF 核 (Radial Basis Function / 高斯核)
═══════════════════════════════════════════════
公式: K(x, y) = exp(-γ × ||x - y||²)
  γ (gamma) = 默认 1/(n_features × X.var())
  ||x - y||² = 两点间距离的平方

计算 (假设 γ = 0.5):
  ||x - y||² = (1-3)² + (2-1)² = 4 + 1 = 5
  K(x, y) = exp(-0.5 × 5) = exp(-2.5) ≈ 0.082

  → 距离近的两个点: x=[1,2], y=[1,3]
    ||x-y||² = 0 + 1 = 1
    K = exp(-0.5 × 1) = exp(-0.5) ≈ 0.607  ← 相似度高

  → 距离远的两个点: x=[1,2], y=[10,10]
    ||x-y||² = 81 + 64 = 145
    K = exp(-0.5 × 145) ≈ 0.000...         ← 相似度≈0

含义: 近的点相似度高，远的点相似度趋近0
      相当于映射到无限维空间
```
