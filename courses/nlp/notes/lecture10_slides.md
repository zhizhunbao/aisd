...
> **📝 Notes:**
>
> **承接**: 本节作为开篇，列出本课的两大主题——BERT 架构与问答系统；这些议程将为下一节「Transformer 发展树」提供学习路线图。
...
> **📝 Notes:**
>
> **承接**: 上一节列出了课程议程；本节通过 Transformer 发展树和时间线，定位 BERT 在整个 NLP 技术演化中的位置；这一背景为下一节深入 BERT 架构打下基础。
...
> **📝 Notes:**
>
> **承接**: 上一节介绍了 BERT 在 Transformer 家族中的位置；本节详解 BERT 的模型架构（Base 与 Large 配置、参数规模）；理解架构细节是下一节学习 BERT 输入输出机制的前提。
...
> **📝 Notes:**
>
> **承接**: 上一节介绍了 BERT 的模型架构；本节详解 BERT 的输入处理流程（WordPiece 分词、特殊 Token、嵌入层）和输出表示（[CLS] 用于整句表示）；这些输入输出机制是下一节理解 BERT 训练方法的基础。
...
> **📝 Notes:**
>
> **承接**: 上一节介绍了 BERT 的输入输出机制；本节详解 BERT 的两大预训练任务——MLM（掩码语言模型）和 NSP（下一句预测），以及总损失的计算方式；理解预训练方法是下一节学习迁移学习与微调的前提。
...
> **📝 Notes:**
>
> **承接**: 上一节介绍了 BERT 的预训练方法；本节讲解迁移学习的思路（预训练→微调范式）；理解迁移学习是下一节 BERT 应用于文本分类和了解 BERT 变体的基础。
...
> **📝 Notes:**
>
> **承接**: 上一节介绍了迁移学习与微调范式；本节展示 BERT 在文本分类中的具体应用，并列举主要的 BERT 变体（如 DistilBERT）；这些应用场景和变体为下一节引入问答系统（QA）做好铺垫。
...
> **📝 Notes:**
>
> **承接**: 上一节完成了 BERT 部分的全部内容；本节从宏观视角介绍问答系统的分类法（信息源、问题类型、答案类型、QA 范式）；这一分类框架为下一节深入抽取式 QA 提供概念基础。
...
> **📝 Notes:**
>
> **承接**: 上一节介绍了 QA 系统的分类框架；本节聚焦抽取式 QA / 阅读理解——从 IBM Watson 到 SQuAD 数据集及其评估指标（EM、F1）；这些基准和指标是下一节学习神经阅读理解模型的评价标准。
...
> **📝 Notes:**
>
> **承接**: 上一节介绍了 SQuAD 数据集与评估指标；本节详解用于阅读理解的神经模型——从 BiDAF 到 BERT，展示 BERT 如何预测答案跨度的起止位置，并通过 SQuAD 2.0 排行榜对比各模型性能；这些模型对比为下一节讨论长文本处理策略提供参考。
...
> **📝 Notes:**
>
> **承接**: 上一节对比了阅读理解的各种神经模型；本节讨论长文本处理的滑动窗口策略和 BERT QA 的 tokenizer 实现细节；这些工程实践为下一节的开放域 QA 与信息检索架构做好技术准备。
...
> **📝 Notes:**
>
> **承接**: 上一节讨论了 BERT QA 的工程实现；本节扩展到开放域 QA，介绍检索器-阅读器架构、文档存储方式、嵌入方法（Word2Vec、GloVe、BERT）以及密集段落检索 DPR 的双编码器架构；这些检索技术为下一节的 QA 框架与评估提供核心组件。
...
> **📝 Notes:**
>
> **承接**: 上一节介绍了开放域 QA 的检索架构；本节介绍 Haystack 等 QA 框架的实际使用以及阅读器的评估方法（EM、F1）；这些框架和评估方法为下一节「超越抽取式 QA」提供工具基础。
...
> **📝 Notes:**
>
> **承接**: 前面各节完成了从 BERT 架构到抽取式 QA 框架的全流程；本节作为结尾，展望 RAG（检索增强生成）和 LLM 的发展方向，为后续课程的 LLM 主题做引导。
