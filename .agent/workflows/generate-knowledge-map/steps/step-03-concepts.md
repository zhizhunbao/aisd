...
| `> 别名` 标注 | 同一概念在不同领域的不同名字，标注每个名字的来源领域。⚡ **同时作为视频叙事素材** — "为什么同一个东西有这么多名字"是天然的故事钩子 | Wüster《Introduction to General Terminology Theory》(1979) — 术语标准化要求追溯同义词的学科来源，消除跨领域沟通歧义 |
...
> 别名：**Name2**（来自XX领域）/ **Name3**（来自YY领域）— 说明为什么不同领域起了不同名字
...
- ✅ 同概念多名字时，用 `> 别名` 标注所有名字及其来源领域（如 Recall = Sensitivity = TPR，分别来自信息检索/医学/信号检测论）
- ✅ 概念辨析用对比表格
- ✅ 核心属性有 信息架构 + 适用✅ + 不适用❌
- ❌ 不要用表格列术语定义
