---
name: learning-automated-study-material
description: Transform raw course materials (PPT/PDF) into an interactive Jupyter Notebook learning package. Orchestrates demo scripting, notebook synthesis, and quality review.
---
...
Raw course materials (PPTX/PDF) must be converted to readable markdown.
...
### Phase 1: Implementation Demo (`*_slides.md` -> `*_complete_demo.py`)

**Uses:** `learning-code_generation`

- Read the slides markdown (from Phase 0) to understand the concepts.
...
- Verify the script runs end-to-end and the logic matches the theory from the slides.

**Output:** `[topic]_complete_demo.py` + `[topic]_complete_demo_pages/`

### Phase 2: Interactive Synthesis (`*_complete_demo.py` -> `*_complete_demo.ipynb`)

**Uses:** `learning-notebook_conversion`

- Merge theory (from slides) and code (Phase 1) into an interactive Jupyter Notebook.
- Cell structure for each concept:
  1. **Markdown Cell**: Brief introduction + simple explanation of the problem.
  2. **Markdown Cell**: Theory (from slides) + defined terms.
  3. **Code Cell**: Implementation code (from Phase 1).
...
**Output:** `[topic]_complete_demo.ipynb`

### Phase 3: Execution, Verification & Review
...
#### 3b. Logic Consistency Review (`learning-logic_consistency`)
...
#### 3c. Code & Asset Consistency Review (`learning-code_consistency`)

- Code logic in the notebook matches the demo script from Phase 1.
...
### Phase 4: Quiz Generation (`*_slides.md` + `*_complete_demo.py` -> `*_quiz.md`)

**Uses:** `learning-quiz_generation`

- Analyze the slides and demo code to create a comprehensive self-assessment.
...
└── notes/
    ...
    │   ├── slide1_img1.png
    │   └── slide2_img1.png
    ├── [topic]_complete_demo.py          # Phase 1: Code (Validation Script)
    ├── [topic]_complete_demo_pages/      # Phase 1: Assets (Reference Images)
    │   ├── [topic]_demo_plot1.png
    │   └── [topic]_demo_plot2.png
    ├── [topic]_complete_demo.ipynb       # Phase 2: The Final Experience
    └── ../quizzes/
        └── [topic]_quiz.md              # Phase 4: Knowledge Assessment
```
