...
**Tags:** `#statistics` `#probability` `#ml-week5` `#ml-week6`
...
- ML Week 5 §2 (Bayes Classifier — conditional probability and Bayes theorem)
- ML Week 5 §3 (MAP decision rule — dropping P(X) for comparison)
...
- Σ\_{x∈Cᵢ} = sum over all points x in cluster i
...
### Laplace Smoothing / m-estimate (拉普拉斯平滑 / m-估计)

**Tags:** `#statistics` `#probability` `#smoothing` `#ml-week5`

**📌 One-line Definition:**

> Laplace smoothing prevents zero probabilities in Naïve Bayes by adding virtual observations to every attribute-class combination.
>
> > 拉普拉斯平滑通过向每个属性-类别组合添加虚拟观测来防止朴素贝叶斯中的零概率。

**📐 Formula:**

```
Laplace:    P(Xᵢ=c|y) = (nc + 1) / (n + v)
...
- m = equivalent sample size for prior confidence
- Laplace is special case: m=v, p=1/v

**💡 Intuition (直觉理解):**

> **Restaurant review analogy:** Every new restaurant starts with 1 fake positive + 1 fake negative review. Even with zero real reviews, the rating is 50% (not 0% or 100%). As real reviews accumulate, fake ones get diluted.
>
> > **餐厅评价类比：** 每家新餐厅默认有1个假好评+1个假差评。即使没有真实评价，评分也是50%（而非0%或100%）。随着真实评价积累，假评价被稀释。

**🔢 Worked Example:**

```
nc=0, n=3, v=3 (e.g., 3 possible marital statuses)

Original:  P = 0/3 = 0     ❌ zero!
Laplace:   P = (0+1)/(3+3) = 1/6 ≈ 0.167  ✅
```

**🔗 Related Concepts:**
→ see: Bayes' Theorem (Laplace fixes zero probabilities in NB)
→ see: Naïve Bayes (the classifier that needs smoothing)

**📚 Appears In:**

- ML Week 5 §7 (Issues with Naïve Bayes — zero probability problem and smoothing solutions)
